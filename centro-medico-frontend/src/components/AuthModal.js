    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isLogin) {
                ...
                // Register flow in Firebase
                const userCredential = await createUserWithEmailAndPassword(auth, email, password);
                const token = await userCredential.user.getIdToken();
                localStorage.setItem('token', token);

                // Save basic user in our backend without profile details
            ...
            // Firebase error messages are usually in English, we can map common ones
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 relative max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    {/* Removed extra inputs (Name, Surname, DNI, BirthDate) to simplify registration.
                        These will form part of PatientProfile completion on first appointment scheduling. */}
                    ...
                    <div className="relative flex items-center justify-center w-full mt-4 mb-4">
                    ...
                    <button
                        ...
                        disabled={loading}
                        className="w-full bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold py-2 rounded-lg transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                    >
